...
func run(out io.Writer, redisURL, postgresDSN string) error {
	...
	defer func() { _ = repo.dropTable(context.Background(), db) }()
	...
	if state.Status != "completed" || state.FencingToken != 2 || state.OwnerID != secondReq.Ownership.OwnerID || state.LockID != exampleDefinitionID {
		return fmt.Errorf(

...
func run(out io.Writer, redisURL string) error {
	...
	store := redis.New(client, prefix+":idempotency")
	...
	if err := mgr.ExecuteClaimed(context.Background(), req, func(ctx context.Context, claim definitions.ClaimContext) error {
		...
		presence, err := driver.CheckPresence(ctx, backend.PresenceRequest{
	...
	presenceAfterRelease, err := driver.CheckPresence(context.Background(), backend.PresenceRequest{
		DefinitionID: req.DefinitionID,

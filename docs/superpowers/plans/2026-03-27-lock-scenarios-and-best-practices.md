...
- `Inventory Reservation From A Queue Worker`: explicitly state that `workers` plus `idempotency` are recommended, and that `async delivery semantics matter as much as locking`

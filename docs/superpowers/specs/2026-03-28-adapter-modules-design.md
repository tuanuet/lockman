...
Second, historical and engine-level examples still import those low-level adapter packages directly, which keeps the internal engine boundary blurry even though the new first-contact SDK path already prefers `github.com/tuanuet/lockman/backend/redis` and `github.com/tuanuet/lockman/idempotency/redis`.
...
- `adapters/redis` imports `github.com/tuanuet/lockman/lockkit/drivers`
- `adapters/idempotency/redis` imports `github.com/tuanuet/lockman/lockkit/idempotency`
- `adapters/guard/postgres` imports `github.com/tuanuet/lockman/guard`
...
The root package `github.com/tuanuet/lockman/backend/redis` should no longer be a thin wrapper around an internal `lockkit` package. It becomes the adapter module itself.

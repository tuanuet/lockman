...
On that root SDK path, use `DefineLock` plus `DefineRunOn`, `DefineHoldOn`, or `DefineClaimOn` for new code.

Advanced packages such as `advanced/strict` and `advanced/composite` are specialized surfaces with their own authoring models.
